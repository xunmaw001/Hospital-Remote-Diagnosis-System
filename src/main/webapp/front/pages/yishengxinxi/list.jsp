        ...
        <fieldset class="search-container" style="text-align: center;margin: 30px 0;">
          <form class="layui-form">
            ...
            <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
              ...
              <button v-if="isAuth('yishengxinxi','新增')" @click="jump('../yishengxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
        ...
        <div class="pro-lists-con">
          <ul class="after">
            <li v-for="(item,index) in dataList" v-bind:key="index">
              <a target="_self" :href="'../yishengxinxi/detail.jsp?id='+item.id">
                <div class="posi-img">
                  <img :src="item.touxiang?item.touxiang.split(',')[0]:''" width="385px" height="220px" />
                </div>
                <div class="remark">
                  <h5>{{item.yishengxingming}}</h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#yishenggonghao').val()) {
          param['yishenggonghao'] = jquery('#yishenggonghao').val() ? '%' + jquery('#yishenggonghao').val() + '%' : '';
        }  
        ...
        if (jquery('#zhicheng').val()) {
          param['zhicheng'] = jquery('#zhicheng').val() ? '%' + jquery('#zhicheng').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('yishengxinxi/list', 'get', param, function(res) {
