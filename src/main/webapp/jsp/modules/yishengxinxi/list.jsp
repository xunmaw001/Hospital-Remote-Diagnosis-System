...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">医生信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active">医生信息列表</li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">医生信息列表</h3>							
												...
												<div class="custom-control custom-checkbox">
													...
													<label class="custom-control-label" for="select-all"></label>
																																						...
																																						<th onclick="sort('yishenggonghao')">医生工号<i id="yishenggonghaoIcon" class="fa fa-sort"></i></th>
																																																			...
																																																			<th onclick="sort('yiling')">医龄<i id="yilingIcon" class="fa fa-sort"></i></th>
																																																			
																													<th>头像</th>
																																																																	<th onclick="sort('jianjie')">简介<i id="jianjieIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('yuyuefeiyong')">预约费用<i id="yuyuefeiyongIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			row.appendChild(checkbox)
							...
							var yishenggonghaoCell  = document.createElement('td');
									...
									row.appendChild(xingbieCell);
				            				var yilingCell  = document.createElement('td');

												 yilingCell.innerHTML = item.yiling;

				
									row.appendChild(yilingCell);
				  ...
				  touxiangCell.appendChild(touxiangImg);
									...
									row.appendChild(touxiangCell);
				            				var jianjieCell  = document.createElement('td');

												  //如果图文详情包含图片
				  if(item.jianjie && item.jianjie.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.jianjie.indexOf('<img');
					var endIndex = item.jianjie.indexOf('>');				
					jianjieCell.innerHTML = item.jianjie.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.jianjie != null && item.jianjie != "" && item.jianjie.length >= 11){
				  		jianjieCell.innerHTML = item.jianjie.substring(0, 10) + "..."; 
				  	}else{
				  		jianjieCell.innerHTML = item.jianjie;
				 	}
				  }
				  

                
									row.appendChild(jianjieCell);
				            				var yuyuefeiyongCell  = document.createElement('td');

												 yuyuefeiyongCell.innerHTML = item.yuyuefeiyong;

				
									row.appendChild(yuyuefeiyongCell);
            ...
            crossBtn1.innerHTML = "预约"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"yishengyuyue\")");
			if(crossBtnControl('预约') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yishengxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"yishengxinxi");
